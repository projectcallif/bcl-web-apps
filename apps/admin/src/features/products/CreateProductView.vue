...
import { ArrowLeft, Check, Percent, Plus, Trash2 } from "lucide-vue-next";
  ...
  <div class="flex flex-col gap-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
      ...
      <BaseButton variant="primary" @click="handleSubmit" :loading="saving">
        <Check class="w-4 h-4" /> Launch Product
      </BaseButton>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <!-- General Info -->
      <div
        class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8 md:col-span-2"
      >
        <h3
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div class="sm:col-span-2">
            ...
            <AppTextInput
              v-model="form.name"
              id="name"
              placeholder="Personal Loan, Salary Advance, etc."
            />
          </div>
          <div class="sm:col-span-2">
            <label
      ...
      <div class="bg-slate-900 rounded-3xl p-6 sm:p-8 text-white">
        <div class="flex items-center gap-2 mb-6">
          ...
          <h3 class="text-xs font-bold text-white/60 uppercase tracking-widest">
            Base Rates
          </h3>
        </div>
        <div class="space-y-4">
          <div>
            ...
            <AppTextInput
              ...
              id="int"
              type="number"
              class="bg-white/10 border-white/10 text-white"
            />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              >
              <AppTextInput
                v-model="form.processingFeeRate"
                id="p"
                type="number"
                class="bg-white/10 border-white/10 text-white"
              />
      ...
      >
        <div class="flex items-center justify-between mb-6">
          ...
          >
            Initial Tenors (Optional)
          </h3>
          <BaseButton
            variant="ghost"
            size="sm"
            class="text-primary-600 bg-primary-50"
            @click="addInitialTenor"
          >
        ...
        <div
          v-if="form.tenors.length === 0"
          class="py-12 text-center border-2 border-dashed border-slate-100 rounded-2xl"
        >
          <p class="text-slate-400 text-sm">
            No initial tenors added. Using fallback limits.
          </p>
        </div>

        <div v-else class="space-y-3">
          <div
            v-for="(tenor, index) in form.tenors"
            :key="index"
            class="flex items-end gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100"
          >
            <div class="flex-1">
              <label
            ...
            <BaseButton
              variant="ghost"
              class="text-rose-600 hover:bg-rose-50 px-2"
              @click="removeInitialTenor(index)"
            >
