...
const filterBadgeClass = computed(() => (value: LoanStatus | null) =>
...
async function fetchLoans() {
  ...
  const res = await getLoans({ page: currentPage.value, pageSize: PAGE_SIZE, status: statusFilter.value })
...
getLoanStats().then(res => { stats.value = res.data })
...
<template>
  <div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <RouterLink
        :to="{ name: 'loan-apply' }"
        class="flex items-center gap-2 px-4 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-800 transition-colors shadow-sm shadow-primary/20"
      >
        <Plus class="w-4 h-4" />
        Apply for Loan
      </RouterLink>
    ...
    <div v-if="stats" class="grid grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm shadow-slate-200/50 p-5">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ formatCurrency(stats.totalOutstanding) }}</p>
    ...
    <div v-else-if="loans.length === 0" class="bg-white rounded-2xl border border-slate-100 p-12 text-center">
    ...
    <!-- Pagination -->
    <div v-if="!isLoading && totalPages > 1" class="bg-white rounded-2xl border border-slate-100 shadow-sm shadow-slate-200/50 px-6 py-4">
      <AppPagination
    ...
    <!-- Apply CTA banner -->
    <div class="bg-gradient-to-r from-primary to-primary-800 rounded-2xl p-6 flex items-center justify-between overflow-hidden relative">
      ...
      <RouterLink
        :to="{ name: 'loan-apply' }"
        class="relative flex items-center gap-2 px-5 py-2.5 bg-white text-primary text-sm font-bold rounded-xl hover:bg-primary-50 transition-colors shrink-0"
      >
